...
camera_pos = (0, -25, 15)
cannon_angle = 0
car_x, car_y, car_z = 0, 0, 5
player_speed = 5.0
lane_positions = [-30, -15, 0, 15, 30]
fovY = 60
GRID_LENGTH = 600
...
class EnemyCar:
    ...
    def move(self):
        ...
        dx = car_x - self.x
        dy = car_y - self.y
        self.enemy_cannon_angle = degrees(atan2(dx, dy)) + random.uniform(-5, 5)
...
def keyboard(key, x, y):
    ...
    if key == 'z':
        ...
        car_lane = min(4, car_lane + 1)
    elif key == 'w':
        moving_forward = not moving_forward
        if moving_forward:
            moving_backward = False
    elif key == 's':
        moving_backward = not moving_backward
        if moving_backward:
            moving_forward = False
    elif key == 'v':
...
def draw_car():
    ...
    gluSphere(quadric, 0.4, 10, 10)
    glPopMatrix()
    # Draw wheels using spheres instead of torus
    glColor3f(0.1, 0.1, 0.1)
    glPushMatrix()
